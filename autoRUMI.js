        ...
        const RUMICommentAnalyzer = {
            ...
            checkTriggerPhrases(commentBody, htmlBody, comment) {
                ...
                for (let phraseIndex = 0; phraseIndex < rumiEnhancement.pendingTriggerPhrases.length; phraseIndex++) {
                   ...
                   if (foundMatch) {
    // NEW REQUIREMENT: Check if comment contains "careem actions required on rider" or "careem actions required for rider" - if so, exclude from pending
    const careemExclusionPhrases = [
        "careem actions required on rider",
        "careem actions required for rider" // Added new exclusion phrase
    ];
    
    // Function to check if a body (commentBody or htmlBody) contains any of the exclusion phrases
    const containsExclusion = (body) => {
        if (!body) return false;
        const lowerBody = body.toLowerCase();
        return careemExclusionPhrases.some(phrase => lowerBody.includes(phrase.toLowerCase()));
    };

    const containsCareemExclusion = containsExclusion(commentBody) || containsExclusion(htmlBody);

    if (containsCareemExclusion) {
        RUMILogger.info('COMMENT', `Found matching phrase (${matchType}) but comment also contains a Careem exclusion phrase - excluding from pending`, {
            phrase: phrase.substring(0, 50) + '...',
            commentId: comment.id,
            authorId: comment.author_id,
            matchType: matchType,
            matchDetails: matchDetails,
            exclusionReason: 'Contains Careem Actions Required on Rider' // You might keep this reason or make it more generic
        });
